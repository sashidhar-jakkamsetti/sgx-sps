...
SGX_SDK ?= /home/sashi/RemoteAttestation/sgx/sgxsdk
SGX_MODE ?= SIM
SGX_ARCH ?= x64
SGX_DEBUG ?= 1
...
App_Cpp_Files := app/app.cpp app/recipient.cpp app/test.cpp app/utils.cpp app/storage/storage.cpp app/crypto/crypto.cpp
App_Include_Paths := -Iinclude -Iapp -Iapp/storage -Iapp/crypto -I$(SGX_SDK)/include
...
App_Link_Flags := -L$(SGX_LIBRARY_PATH) -l$(Urts_Library_Name) -lcrypto -lpthread -L$(SGXSSL_UNTRUSTED_LIB_PATH) -lsgx_usgxssl
...
Enclave_Cpp_Files := enclave/enclave.cpp enclave/crypto/crypto.cpp enclave/oram/block.cpp enclave/oram/bucket.cpp enclave/oram/oram.cpp
Enclave_Include_Paths := -Ienclave -Ienclave/oram -Ienclave/crypto -Iinclude -I$(SGX_SDK)/include -I$(SGX_SDK)/include/tlibc -I$(SGX_SDK)/include/libcxx -I$(SGXSSL_INCLUDE_PATH)
...
SGXSSL_Enclave_Link_Flags := -Wl,--whole-archive -lsgx_tsgxssl -Wl,--no-whole-archive -lsgx_tsgxssl_crypto -lsgx_pthread
...
Enclave_Link_Flags := $(MITIGATION_LDFLAGS) $(Enclave_Security_Link_Flags) $(SGXSSL_Enclave_Link_Flags)\
    -Wl,--no-undefined -nostdlib -nodefaultlibs -nostartfiles -L$(SGX_TRUSTED_LIBRARY_PATH) -L$(SGXSSL_TRUSTED_LIB_PATH) \
