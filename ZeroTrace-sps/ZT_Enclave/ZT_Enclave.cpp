...
int sps_ecall_init_service(int msg_nblocks, int msg_blocksize, int msg_stashsize, int msg_recursion_levels, 
	...
	int oram_ret = 0;
	if (!is_init) {
    // ocall_print_string("\nService: initiating service and creating ORAMs for messages and recipient DBs\n");

    uint32_t msg_id = 0;
    oram_ret = createNewORAMInstance(msg_id, msg_nblocks, msg_blocksize, msg_stashsize, 1, recursion_data_size, msg_recursion_levels, oram_type, bucket_size);

    // ocall_print_string("Service: initialized messages DB\n");

    uint32_t recp_id = 1;
    oram_ret = createNewORAMInstance(recp_id, recp_nblocks, recp_blocksize, recp_stashsize, 1, recursion_data_size, recp_recursion_levels, oram_type, bucket_size);

		// ocall_print_string("Service: initialized recipients DB\n");

    if(oram_type==0){
      sps_orams.msg = poram_instances[msg_id];
      sps_orams.recp = poram_instances[recp_id];
    }
    else {
      sps_orams.msg1 = coram_instances[msg_id];
      sps_orams.recp1 = coram_instances[recp_id];
    }
		
		EC_KEY* keypair1 = generate_ec_keypair();
		sps_keys.enck = extract_ec_pubkey(keypair1);
		sps_keys.deck = extract_ec_privkey(keypair1);
		EC_KEY_free(keypair1);

		EC_KEY* keypair2 = generate_ec_keypair();
		sps_keys.verk = extract_ec_pubkey(keypair2);
		sps_keys.sigk = extract_ec_privkey(keypair2);
		EC_KEY_free(keypair2);

		RSA* keypair3 = generate_rsa_keypair();
		sps_keys.enck_send = extract_rsa_pubkey(keypair3);
		sps_keys.deck_send = extract_rsa_privkey(keypair3);
		RSA_free(keypair3);
		// ocall_print_string("Service: enclaves keys generated\n");

		is_init = true;
		ctr_recip = 1;
		ctr_msg = 1;
		// ocall_print_string("Service: service is up\n");
	}
	else {
		// ocall_print_string("Service: already initialized\n");
...
int sps_ecall_get_keys(void* out_data, size_t out_size) {
	string debug_str = "";

	if (out_size != sizeof(keys_output_t)) {
		return ERROR_INVALID_ARGUMENTS;	
	}
	// ocall_print_string("Service: sending enclave keys\n");

	keys_output_t out_get_keys_data;
	int buffer_len1 = 0;
	unsigned char *buffer1 = convert_ec_pubkey_2_buffer(sps_keys.enck, &buffer_len1);
	
	if (!buffer1 || buffer_len1 != EC_PK_SIZE_BYTES) {
        // ocall_print_string("Service: Public key len is not equal to RECIPIENT_PK_SIZE_BYTES\n");
		return ERROR_VARIABLE_SIZE_INCONSISTENT;
    }
	memcpy(out_get_keys_data.pk, buffer1, buffer_len1);

	int buffer_len2 = 0;
	unsigned char *buffer2 = convert_ec_pubkey_2_buffer(sps_keys.verk, &buffer_len2);
	if (!buffer2 || buffer_len2 != EC_PK_SIZE_BYTES) {
        // ocall_print_string("Service: Verification key len is not equal to RECIPIENT_VK_SIZE_BYTES\n");
		return ERROR_VARIABLE_SIZE_INCONSISTENT;
    }
	memcpy(out_get_keys_data.vk, buffer2, buffer_len2);

	int buffer_len3 = 0;
	unsigned char *buffer3 = convert_rsa_pubkey_2_buffer(sps_keys.enck_send, &buffer_len3);
	if (!buffer3 || buffer_len3 != RSA_PK_SIZE_BYTES) {
        // ocall_print_string("Service: Encryption key (send) len is not equal to SERVICE_PK_SIZE_BYTES\n");
		return ERROR_VARIABLE_SIZE_INCONSISTENT;
    }
	memcpy(out_get_keys_data.service_pk, buffer3, buffer_len3);

	memcpy(out_data, &out_get_keys_data, sizeof(keys_output_t));
	// ocall_print_string("Service: written enclave keys\n");
	return RET_SUCCESS;
}


int sps_ecall_setup(void* in_data, size_t in_size, void* out_data, size_t out_size, 
	...
	unsigned char* plaintext = decrypt_blob_rsa_pubkey((unsigned char*)in_data, in_size, 
			&plaintext_len, sps_keys.deck_send);
	...
	// ocall_print_string("Service: setup phase for user\n");

	unsigned char data[ORAM_BLOCK_SIZE_RECP];
	unsigned char data_out[ORAM_BLOCK_SIZE_RECP];
	clearData(data, 0, ORAM_BLOCK_SIZE_RECP);
	// clearData(data_out, 0, ORAM_BLOCK_SIZE_RECP);
	...
	offset += EC_PK_SIZE_BYTES;
	sps_orams.recp->Access(setup_in_data->rid, 'w', data, data_out);
	ctr_recip += 1;

	// ocall_print_string("Service: wrote keys and counter for user to keys ORAM\n");
	...
	unsigned char* signature = ecdsa_sign((unsigned char*)out_data, 
		out_size, &signature_len, sps_keys.sigk);

	if (signature_len + sizeof(int) > ECDSA_SIG_SIZE_BYTES) {
        // ocall_print_string("Service: Signature len is not less than ECDSA_SIG_SIZE_BYTES\n");
		return ERROR_VARIABLE_SIZE_INCONSISTENT;
    }
  ...
  memcpy(sig_data + sizeof(int), signature, signature_len);
	...
	// ocall_print_string("Service: output written\n");
	delete[] plaintext;
	return RET_SUCCESS;
}


int sps_ecall_send(void* in_data, size_t in_size, void* out_data, size_t out_size,
				void* sig_data, size_t sig_size) {
	...
	// ocall_print_string("Service: send phase for user\n");

	unsigned char recip_data[ORAM_BLOCK_SIZE_RECP];
	unsigned char recip_data_in[ORAM_BLOCK_SIZE_RECP];
	clearData(recip_data, 0, ORAM_BLOCK_SIZE_RECP);
	sps_orams.recp->Access(send_in_data->rid, 'r', recip_data_in, recip_data);
	...
	// ocall_print_string("Service: read previous data for user from recipient DB\n");

	unsigned char message_data[ORAM_BLOCK_SIZE_MSG];
	unsigned char message_data_out[ORAM_BLOCK_SIZE_MSG];
	clearData(message_data, 0, ORAM_BLOCK_SIZE_MSG);
	memcpy(message_data, &prev_idx, sizeof(int));  // Previous idx
	memcpy(message_data + sizeof(int), &prev_loc, sizeof(int));  // Previous loc
	sps_orams.msg->Access(ctr_msg, 'w', message_data, message_data_out);

	memcpy(recip_data, &ctr_msg, sizeof(int));
	memcpy(recip_data + sizeof(int), &send_in_data->loc, sizeof(int));
	sps_orams.recp->Access(send_in_data->rid, 'w', recip_data, recip_data_in);
	ctr_msg += 1;
...
int sps_ecall_receive(void* in_data, size_t in_size, void* out_data, size_t out_size,
				void* sig_data, size_t sig_size) {
	...
	// ocall_print_string("Service: receive phase for user\n");

  unsigned char data[ORAM_BLOCK_SIZE_RECP];
  unsigned char recip_data_in[ORAM_BLOCK_SIZE_RECP];
  clearData(data, 0, ORAM_BLOCK_SIZE_RECP);
	sps_orams.recp->Access(receive_in_data->rid, 'r', recip_data_in, data);
  
  unsigned char recip_data[ORAM_BLOCK_SIZE_RECP];
  memcpy(recip_data, data, ORAM_BLOCK_SIZE_RECP);

	// ocall_print_string("Service: read keys and counter for user from recipient DB\n");
	int cur_ctr = 0;
	unsigned int cur_idx = 0, cur_loc = 0;
  ...
  EC_KEY* recip_vk = convert_buffer_2_ec_pubkey(recip_vk_buffer, EC_PK_SIZE_BYTES);
	...
	// ocall_print_string("Service: extracted recipient keys\n");
	...
	if (cur_ctr != receive_in_data->counter || verify_res != 1) {
		// ocall_print_string("Service: user authentication failed: either counter or signature is wrong\n");
  ...
  unsigned char msg_data_in[ORAM_BLOCK_SIZE_MSG];
  unsigned char msg_data_out[ORAM_BLOCK_SIZE_MSG];

	int count = 0;
	while (count < max_count) {

    receive_out_data[count++] = cur_idx == 0? -1: cur_loc;
		clearData(msg_data_out, 0, ORAM_BLOCK_SIZE);
		sps_orams.msg->Access(cur_idx, 'r', msg_data_in, msg_data_out);
		memcpy(&cur_idx, msg_data_out, sizeof(int));
		memcpy(&cur_loc, msg_data_out + sizeof(int), sizeof(int));
    cur_idx = cur_idx == -1 ? 0 : cur_idx;
	}

	receive_out_data[max_count] = cur_idx > 0 ? -2 : -1;
	...
	sps_orams.recp->Access(receive_in_data->rid, 'w', recip_data, data);
	// ocall_print_string("Service: wrote index and counter for user to recipient DB\n");
	...
	unsigned char* ciphertext = ec_encrypt(plaintext_out, plaintext_len_out, &ciphertext_len, 
		sps_keys.deck, recip_pk);

	if (ciphertext_len > out_size) {
        // ocall_print_string("Service: receive out_size is less than ciphertext_len\n");
		return ERROR_VARIABLE_SIZE_INCONSISTENT;
    }
...
int sps_ecall_kill_service() {

	if (is_init) {
		// ocall_print_string("Service: exiting service and destroying ORAMs\n");
    poram_instance_id = 0;
    coram_instance_id = 0;
    poram_instances.clear();
    coram_instances.clear();

		EC_KEY_free(sps_keys.enck);
		EC_KEY_free(sps_keys.deck);
		EC_KEY_free(sps_keys.verk);
		EC_KEY_free(sps_keys.sigk);
		RSA_free(sps_keys.enck_send);
		RSA_free(sps_keys.deck_send);
		is_init = false;
	}
	else {
		// ocall_print_string("Service: not yet initialized\n");
