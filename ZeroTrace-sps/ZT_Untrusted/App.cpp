...
int8_t SPS_init_service(int msg_nblocks, int msg_blocksize, int msg_stashsize,  
                        int recp_nblocks, int recp_blocksize, int recp_stashsize, 
                        int recursion_data_size, int bucket_size, uint32_t oram_type) {
  
  int ret;
  sgx_status_t sgx_return = SGX_SUCCESS;
  uint32_t msg_instance_id = 0, recp_instance_id = 0;
  int8_t msg_recursion_levels, recp_recursion_levels;
  uint32_t msg_D, recp_D;
  LocalStorage *msg_ls_oram = new LocalStorage();    
  LocalStorage *recp_ls_oram = new LocalStorage();    

  msg_recursion_levels = computeRecursionLevels(msg_nblocks, recursion_data_size, MEM_POSMAP_LIMIT);
  msg_D = (uint32_t) ceil(log((double)msg_nblocks/bucket_size)/log((double)2));
  msg_ls_oram->setParams(msg_nblocks,msg_D,bucket_size,msg_stashsize,msg_blocksize + ADDITIONAL_METADATA_SIZE,inmem_flag, 
         recursion_data_size + ADDITIONAL_METADATA_SIZE, msg_recursion_levels);
  
  getNewORAMInstanceID(global_eid, &msg_instance_id, oram_type);
  
  recp_recursion_levels = computeRecursionLevels(recp_nblocks, recursion_data_size, MEM_POSMAP_LIMIT);
  recp_D = (uint32_t) ceil(log((double)recp_nblocks/bucket_size)/log((double)2));
  recp_ls_oram->setParams(recp_nblocks,recp_D,bucket_size,recp_stashsize,recp_blocksize + ADDITIONAL_METADATA_SIZE,inmem_flag, 
         recursion_data_size + ADDITIONAL_METADATA_SIZE, recp_recursion_levels);

  getNewORAMInstanceID(global_eid, &recp_instance_id, oram_type);

  printf("INSTANCE_IDs returned (msg, recp) = (%d, %d)\n", msg_instance_id, recp_instance_id);  

  if(oram_type==0){
    ls_PORAM.insert(std::make_pair(msg_instance_id, msg_ls_oram));
    ls_PORAM.insert(std::make_pair(recp_instance_id, recp_ls_oram));
  }
  else if(oram_type==1) {
    ls_CORAM.insert(std::make_pair(msg_instance_id, msg_ls_oram));
    ls_CORAM.insert(std::make_pair(recp_instance_id, recp_ls_oram));
  }
  
  sgx_return = sps_ecall_init_service(global_eid, &ret, 
                                    msg_nblocks, msg_blocksize, msg_stashsize, msg_recursion_levels, 
                                    recp_nblocks, recp_blocksize, recp_stashsize, recp_recursion_levels, 
                                    recursion_data_size, bucket_size, oram_type);
  if (sgx_return != SGX_SUCCESS){
    printf("Failed to initialize SPS service!\n");
    return -1;
  }
...
int8_t SPS_get_keys(void* out_data, size_t out_size) {
  int ret;
  sgx_status_t sgx_return = SGX_SUCCESS;
  sgx_return = sps_ecall_get_keys(global_eid, &ret, out_data, out_size);
  if (sgx_return != SGX_SUCCESS){
    printf("Failed to get_keys SPS service!\n");
    return -1;
  }
  return 0;
}

int8_t SPS_setup(void* in_data, size_t in_size, void* out_data, size_t out_size, 
		              void* sig_data, size_t sig_size) {
  int ret;
  sgx_status_t sgx_return = SGX_SUCCESS;
  sgx_return = sps_ecall_setup(global_eid, &ret, in_data, in_size, out_data, out_size, 
		              sig_data, sig_size);
  if (sgx_return != SGX_SUCCESS){
    printf("Failed to setup SPS service!\n");
    return -1;
  }
  return 0;
}

int8_t SPS_send(void* in_data, size_t in_size, void* out_data, size_t out_size,
			            void* sig_data, size_t sig_size) {
  int ret;
  sgx_status_t sgx_return = SGX_SUCCESS;
  sgx_return = sps_ecall_send(global_eid, &ret, in_data, in_size, out_data, out_size,
			            sig_data, sig_size);
  if (sgx_return != SGX_SUCCESS){
    printf("Failed to send SPS service!\n");
    return -1;
  }
  return 0;
}

int8_t SPS_receive(void* in_data, size_t in_size, void* out_data, size_t out_size,
			            void* sig_data, size_t sig_size) {
  int ret;
  sgx_status_t sgx_return = SGX_SUCCESS;
  sgx_return = sps_ecall_receive(global_eid, &ret, in_data, in_size, out_data, out_size,
			            sig_data, sig_size);
  if (sgx_return != SGX_SUCCESS){
    printf("Failed to receive SPS service!\n");
    return -1;
  }
  return 0;
}

int8_t SPS_kill_service() {
  int ret;
  sgx_status_t sgx_return = SGX_SUCCESS;
  sgx_return = sps_ecall_kill_service(global_eid, &ret);
  if (sgx_return != SGX_SUCCESS){
    printf("Failed to kill SPS service!");
    return -1;
  }
  ls_PORAM.clear();
  ls_CORAM.clear();
